...
2. `services/showrunner.server.ts#generateShowPlan` orchestrates four Qwen-powered agents. Each agent validates its JSON through `services/showrunner-validator.server.ts` before the next stage runs:
   ...
   - `agents/prompt-agent.server.ts` — scenes → `StoryboardScene[]` (adds a Wan-ready `videoPrompt` per scene). Uses Qwen function-calling (`services/qwen.server.ts#callQwenJson`'s `tools`/`toolHandlers` params) to call a local `get_wan_video_constraints` tool for the real configured resolution/ratio/duration before writing prompts, instead of guessing. Note: Qwen's OpenAI-compatible endpoint hangs indefinitely if `response_format: json_object` is combined with `tools` in the same request (confirmed live) — `callQwenJson` only sets `response_format` on tool-free calls.
