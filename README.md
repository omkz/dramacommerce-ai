...
- `pnpm run test` runs unit tests: the media storage abstraction (local + OSS drivers, against a fake OSS client), the outbox/idempotency behavior (transactional rollback, dispatcher retry/locking, stale-generation guards, cleanup) against a real local Postgres, the shared external-request utility (timeout/abort, bounded reads, streaming-download size limits + partial-file cleanup, HTTP status → retry-category classification, secret sanitization — against local fake HTTP servers, not real provider calls) and its plain-JS mirror, BullMQ's `UnrecoverableError` retry-skipping behavior against a real local Redis, and the domain schema layer (string bounds/whitespace, five-scene invariants and normalization, cross-field brief/storyboard consistency, bounded AI-repair eligibility, queue-payload validation, and corrupted/legacy `show_plan` handling against a real local Postgres). See [External Request Timeouts & Retry Classification](#external-request-timeouts--retry-classification) and [Data Validation & Schema Compatibility](#data-validation--schema-compatibility).
...
## Data Validation & Schema Compatibility

Every AI-generated and merchant-edited shape in the showrunner pipeline — product brief, product analysis, story package, directed/storyboard scenes, critic result, editor package, token usage, and the full show plan — has one canonical Zod schema in `app/services/domain/schemas.server.ts`, built from shared bounds/enums/duration rules in `app/services/domain/limits.server.ts`. `app/types/showrunner.ts` re-exports `z.infer<>` types from these schemas (type-only, erased at compile time — nothing server-only reaches a client bundle) instead of hand-duplicating interfaces; agents, routes, and `project-store.server.ts` all validate against the same schema objects rather than maintaining separate logic per layer.

**Canonical show-plan schema** (`showPlanSchema`) enforces, beyond basic shape:
- every required string is trimmed and rejects whitespace-only input, with an explicit documented max length (centralized in `limits.server.ts`, e.g. `concept` 600 chars, `videoPrompt` 1200 chars, `caption` 2200 chars) — oversized AI output is a validation failure, never silently truncated;
- the storyboard has exactly 5 scenes with integer scene numbers forming exactly `{1,2,3,4,5}` (no duplicates, none missing) — scenes returned out of array order are **normalized** into ascending order via `.transform()` rather than rejected, since a shuffled-but-correct array is a cosmetic defect, not a content defect, and normalizing avoids spending a repair attempt (see below) on it;
- at most one scene has `useProductReference: true`, mirroring the existing `normalizeReferenceSceneUsage` post-processing as defense-in-depth at the schema layer;
- brief-vs-storyboard cross-field invariants: `productReferenceMode: "disable"` means no scene may have `useProductReference: true`; a reference scene requires the brief to actually have a valid, stable managed image reference (`isManagedRef`) — "force" mode overrides the AI's photo-quality judgment, but it can't conjure an image reference that doesn't exist ("fail safely");
- token usage counts are finite non-negative integers, and `totalTokens` is always **normalized** (recomputed as `promptTokens + completionTokens`) rather than rejected on mismatch — it's our own bookkeeping derived from provider usage fields, not model-generated content, so a mismatch is far more likely a provider quirk than a real accounting error.

**Duration representation**: `ProductBrief.duration` stays the display string ("30 seconds") for backward compatibility, but is now a strict enum backed by a canonical `PRODUCT_DURATION_SECONDS` lookup (replacing a duplicated string `Set` that used to live only in `routes/projects.new.tsx`). `DirectedScene.duration` ("0-4s") is cosmetic narrative-range text — confirmed never parsed or computed with anywhere in the app, just displayed and fed into agent prompts — so it stays a validated string format (`SCENE_DURATION_PATTERN`), not a numeric range. The one number that actually determines render length, `WAN_VIDEO_DURATION` (Wan renders every scene at this same fixed duration regardless of narrative pacing), is read through a validated getter (`getWanSceneDurationSeconds()`, same fail-loud-on-invalid-config pattern as `timeout-config.server.ts`). The single shared, duration-aware voice-over-length rule (`getMaxSceneVoiceOverChars()` — 15 chars/sec, an approximate conservative spoken-pace estimate, named accordingly) replaces what used to be two independently-drifted implementations: `routes/projects.$projectId.tsx`'s local `getMaxVoiceOverChars()` and a hardcoded `90` in `video-readiness-skill.server.ts` (which disagreed with the route's own 75-char default). A real mismatch between a merchant's declared total ad duration and Wan's actual fixed per-scene render length (5 × `WAN_VIDEO_DURATION`) is a genuine pre-existing architectural property, not a bug this pass fixes — it's surfaced as a warning through the existing video-readiness skill (same "flag, don't hard-fail" philosophy already used for beat order) rather than a hard schema rejection, which would break every generation.

**Validation boundaries** — data is validated at every point it crosses a trust boundary: incoming merchant form data (`productBriefSchema` in `/projects/new`'s action, replacing ad hoc field checks), AI output after every agent (`validateWithRepair`, see below), data loaded from Postgres JSON columns (`project-store.server.ts`'s `parsePersistedShowPlan`/`parsePersistedProductBrief`, since Drizzle's `$type<ShowPlan>()` is a compile-time cast only — it proves nothing about runtime content), outbox payload deserialization and BullMQ worker payloads (`domain/queue-payload-schemas.server.ts`, mirrored as `scripts/lib/video-job-schemas.mjs` for the plain-JS video worker), merchant story/scene edits (`/projects/:id`'s `edit-story`/`edit-scene` actions, validated through the identical `storyPackageSchema`/`storyboardSceneSchema` the AI output is held to — never a looser check), and immediately before Wan creation (`video-worker.mjs`'s `createWanTask` re-checks the duration-aware voice-over bound and the reference-image invariant against the actual job payload, since a merchant override or a stale scene edit could still violate either even after payload-shape validation passes).

**AI repair policy**: one bounded repair call per agent invocation, scoped *only* to structural Zod failures (wrong type, missing field, a mechanical slip like duplicate/missing scene numbers or a malformed duration string) — never to business-rule violations. Every refinement in `schemas.server.ts` that represents a business rule (voice-over too long for the render duration, a reference-image decision contradicting the brief) is explicitly tagged `params: { repairable: false }`; `agent-json-repair.server.ts#validateWithRepair` checks this tag before ever attempting a repair, and skips straight to failure for any business-rule issue — reprompting "fix your JSON" has no license to rewrite creative content or silently weaken a constraint by treating the model's second guess as authoritative. When eligible, the repair call reuses the same system prompt + original task + the prior malformed JSON + a summary of what to fix, is tracked under the same stage's token usage, and its output is re-validated through the identical schema; if it's still invalid, no second attempt is made — the failure propagates exactly as it did before this existed (job marked `FAILED`, no project created). The cross-agent `showPlanSchema`-level check (brief vs. storyboard) is never repair-eligible by construction — no single agent's repair call could fix a mismatch that requires context (the brief) it doesn't have.

**Schema-version & legacy compatibility**: `ShowPlan` carries a `schemaVersion` field *inside* the JSONB itself — no Drizzle migration, no new column, fully backward compatible (an old row simply lacks the key). Every read validates the full record through `showPlanSchema`; a version field present-but-absent is treated as version 1 via Zod's `.default()` (lazy normalization — the in-memory shape is upgraded on read, persisted for real on the next write), so the version number is a diagnostic/future-proofing marker, not itself a validation gate — the real gate is "does it parse against the current schema." A record that fails validation entirely (corrupted, or a genuinely unsupported future/legacy shape) produces a controlled `schemaStatus: "invalid"` on `SavedProject` rather than crashing: `/projects`, `/dashboard`, and `/projects/:id` all gate on this before touching `.showPlan` fields, and `/projects/:id` renders a dedicated "needs regeneration" screen (delete-only — the merchant is never stuck with an unusable, undeletable row) instead of the normal detail view. No data migration is required for existing rows; the same pattern applies to `showrunner_jobs.brief_json`, which is short-lived enough (read once by the worker moments after being written by the now schema-guarded `/projects/new` action) that an invalid brief simply fails that one job cleanly through the existing `FAILED`-status path, no new UI needed.
